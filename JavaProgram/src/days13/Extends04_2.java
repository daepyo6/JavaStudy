...
//  	- 자식 클래스의 생성자 명령 중 첫줄이 부모 클래스의 생성자 호출이라는 뜻입니다.
//		- 자식 클래스 생성자 명령 첫줄은 super(); 라는 명령이며, 따로 기술하지 않아도 존재하는 명령입니다.
//		- 디폴트 생성자처럼 꺼내서 직접 쓰기 전까지는 숨어있는 명령이라고 할 수 있습니다.
// 4. 부모클래스를 생성자를 모두 실행한 후 자식클래스 생성자의 남은 명령을 실행합니다.
//		- 구조가 이렇게 짜여진 이유를 들자면, 부모클래스의 private 멤버를 자식 클랫의 생성자에서 초기화할 수 없으므로
...
class SuperB{
	...
	SuperB(){
		System.out.println("부모클래스의 디폴트 생성자 실행");
	}
}
class SubB extends SuperB{
	...
	// SubB() { super(); }
	// 이와 같은 생성자와 명령이 숨어 있는 것과 같습니다.
	
	// 생성자를 꺼내서 별도로 정의한 경우 super() 명령 외에 다른 명령도 쓸 수 있습니다.
	SubB(){
		this(10);
		...
		// 혹시라도 부모클래스의 생성자가 매개변수 있는 생성자로 대체되었다면 이는 에러를 발생합니다.
	...
	SubB(int n){		// 오버로딩 된 자식 클래스 생성자
		super(n);
		// 자식 클래스의 오버로딩된 생성자의 첫번째 실행코드는 super();	this(); 가 올 수 있습니다.
		// 한꺼번에 둘 다 올수는 없습니다.
...
// 상속된 자식클래스의 객체가 만들어질 때, 자식클래스의 생성자가 먼저 호출되고, 
// 자식클래스의 첫번째 명령은부모클래스의 생성자 호출입니다.
public class Extends04_2 {
...
// this()와 super() 명령 중 하나를 선택해야하는데, 둘다 안쓰면 super(); 가 자동적용됩니다.
