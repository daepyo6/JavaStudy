...
public class Collection05 {
	...
	public static void main(String[] args) {
		// Key 값은 String, Value값은 Integer형의 자료를 Hashtable에 저장 할 객체를 생성한다면
		Hashtable<String, Integer> ht = new Hashtable<>();
		
		// 같은 제네릭형의 HashMap 생성
		HashMap<Integer, String> hm = new HashMap<>();
		
		// Hashtable, HashMap 클래스는 put 메서드를 사용하여 데이터를 추가
		ht.put("One", 1);
		ht.put("Two", 2);
		ht.put("Three", 3);
		...
		// Hashtable, HashMap 클래스는 get 메서드에 Key 값을 전달하여 Value를 리턴받습니다.
		System.out.println("\"Three\" = " + ht.get("Three"));
		System.out.println("3 = " + hm.get(3));
		
		// 반복문을 이용해서 Hashtable, HashMapdp Value들을 출력하려면, 별도의 객체가 필요합니다.
		// Enumeration 객체
		// Enumeration은 열거형 자료형식(인터페이스)입니다.
		// 구식 방법
		Enumeration<String> e1 = ht.keys(); // 키 값들은 리스트 형식으로 얻어서 저장합니다.
		
		// 객체이름.hasMoreElements() 메서드를 이용해서 다음 키 값이 있는지 확인한 후 마지막 키값까지 하나씩 접근
