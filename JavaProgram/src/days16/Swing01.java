...
class ButtonTest extends JFrame{
	...
	public ButtonTest() {
		...
		// 버튼 또는 그 외의 컨트롤들은 윈도우에 배치되고 위치를 잡아 올라가고 윈도우가 화면에 보여질 때 같이 표시
		
		// 라벨, 이름표, 명찰등의 역할을 하는 안내문구 표시용 텍스트 도구
		JLabel label = new JLabel("당신의 성별은?");
		
		// 윈도우 위에 생성된 컨트롤 객체를 올려놓을 수 있게 해주는 객체 (싱글턴 방식)
		Container con = getContentPane();   // 윈도우에 올라갈 컨드롤들을 담을 바구니 역할을 하는 객체
		// 그 위에 정렬기준을 설정합니다.
		con.setLayout(new FlowLayout());		// 올라가는 순서대로 일직선상에 위치시키는 레이아웃
		
		// 컨테이너에 생성된 컨트롤들을 순서에 맞춰 올려놓으면 화면에 표시될 준비가 완료됩니다.
		con.add(label);
		con.add(male);
		con.add(female);
		
		setTitle("버튼 컴포넌트 테이스");		// 윈도우 좌측 상단에 표시된 타이틀 내용 설정
		setSize(300, 200);					//	윈도우 크기 설정 : 단위(픽셀)
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);	
		// 현재 윈도우 종료시 프로그램까지 종료 설정 
		setVisible(true);	// 화면에 윈도우를 출련시키는 메서드
