...
// 사칙 연산 프로그램
// 두개의 피연산자(정수)와 연산자(연산기호)를 입력 받아서 그 결과를 출력하는 프로그래밍
// 연산했던 기록을 두개의 정수와 하나의 연산기호를 저장할 수 있는 클래스 객체에 저장해서 파일에 저장
// 추후에 파일을 열어 계산했던 기록을 열람할 수 있게 함
// 파일은 날짜를 이용해서 이름을 생성, 검색은 날짜를 입력해서 파일을 검색, 그날 계산했던 기록을 한번에 열람
...
public class IO19 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		// CalculatorReuslt a = new CalculatorReuslt(20, 30, "x", 60000.0);
		...
		while(true) {
			...
			// 좌항입력
			System.out.print("좌항을 입력 : ");
			leftValue = Integer.parseInt(in.readLine());
			// 연산부호 입력
			System.out.println("연산부호 입력 : ");
			operator = in.readLine();
			// trim 메서드는 문자열 앞뒤에 있는 공백을 제거 (문자열 사이의 공백은 제거되지 않음)
			operator = operator.trim();
			// 우항입력
			System.out.println("우항 입력 : ");
			...
			// 파일이름을 오늘날짜현재시간.dat로 생성합니다.  ex) 2022_10_25_17_00.dat
			Date now = new Date();	// 오늘 날짜를 담은 Date 객체 생성
			
			// 파일 이름이 될 양식으로 SimpleDateFormat 객체 생성
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy_MM_dd_HH_mm");
			
			// 오늘 날짜를 sdf에 적용하고 뒤에 ".dat"를 이어붙이기 해서 String데이터로 변경
			 String fileName = sdf.format(now) + ".dat";
			 // 모든 자료형은 String과 이어붙이기하면 결과가 String이 된다.
			 // ex) 2022_10_25_17_00.dat 생성 완료
