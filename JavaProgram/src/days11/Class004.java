...
class Cat {
	...
	int age;
	String phone;
	// 멤버 메서드는 한개 만들어지고, 모든 객체가 그 메서드를 공유해서 사용합니다.
	// 많은 객체가 메서드를 필요할 때마다 호출하는데 그 메서드 입장에서는 그들을, 누가 호출했는지를 어떻게 구분할까
	// 객체 하나가 메서드를 호출하면 눈에 보이지 않게 호출객체의 참조값이 (주소) 전달됩니다.
	// 그 참조값은 눈에 보이지 않는 this라는 참조변수에 저장(전달) 됩니다.
	// c1 이 호출하면 c1 -> this
	...
	public void output(/* Cat this */) {
		System.out.println("이름 : " + this.name + " 나이 : " + age + " 전화번호 : " + this.phone);
		// this 변수는 매개변수와 멤버변수가 이름이 같은 경우 그들을 구분하기 위해 많이 사용합니다.
