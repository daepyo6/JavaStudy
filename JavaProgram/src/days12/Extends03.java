...
class SuperA{
	private int n1;
	// private : 현재 클래스 내에 있는 멤버 메서드를 통해서만 접근 가능한 완전은닉 n1
	public int getN1() {
		return n1;
	}// 자식 클래스에서 만들어진 getN1은 에러이지만, 부모클래스에서 만들어진 getN1은 에러가 아닙니다.
	// 이는 상속되어져서 자식클래스에 있는걸로 인식될 예정이며,
	// public이므로 어디서든 객체가 생성되면 자유롭게 호출이 가능합니다.
	public int n2;  // public : 어디서든 호출이 가능
	
	int n3;
	// 접근 지정을 아무것도 하지 않은 멤버 : default로 지정됩니다.
	// 동일한 패키지 내부에서는 public으로 동작, 다른 패키지에서는 private로 동작합니다.
	
	protected int n4;
	// 자식클래스에서는 public과 같으며, 외부로부터는 private로 작동합니다.
}
...
class SubA extends SuperA{
	...
	public void printInfo() {
		...
		// 부모클래스의 protected는 자식클래스에게 public과 같습니다.
